...
export default function AutomaticScan({url,kind,initial,onAnalysis,onRemove,onKeep}:Props){
 ...
 return <section className="automatic-scan" aria-label="Automatic pet scan">
  ...
  <div className="scan-upload-actions"><label className={`outline-btn scan-add-file ${busy?'disabled':''}`}><Camera size={17}/>{bundle?'Add side / front photo or video':'Choose another view'}<input type="file" aria-label="Add side or front photo or video" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" disabled={busy} onChange={e=>{void addFile(e.target.files?.[0]);e.target.value='';}}/></label>{bundle&&<button className="text-btn" onClick={onKeep}>{bundle.result.status==='ready'?'Review estimated measurements':'Keep this pet profile'}</button>}<button className="text-btn remove-upload-btn" onClick={onRemove}><Trash2 size={16}/>Remove {kind==='video'?'video':'photo'}</button></div>
